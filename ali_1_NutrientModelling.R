...
subdata <- data.frame(sex, age, age1, age2, age3, inc, sector, carb, fat, 
    protein, sodium, energy, energy_original)
...
corrplot(corr = correlation_matrix, type="upper", title=main_title)
...
write.csv(reg_carb$coefficients, "out/boxcox_carb_coefficients.csv")
write.csv(reg_fat$coefficients, "out/boxcox_fat_coefficients.csv")
write.csv(reg_protein$coefficients, "out/boxcox_protein_coefficients.csv")
write.csv(reg_sodium$coefficients, "out/boxcox_sodium_coefficients.csv")

# Save Box-Cox transformation parameters to a CSV file
write.csv(boxcoxparameters, "out/boxcox_parameters.csv")

# Save correlation matrix to a CSV file
write.csv(correlation_matrix, "out/boxcox_correlationmatrix.csv")
